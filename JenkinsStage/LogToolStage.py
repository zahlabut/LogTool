    ...
    def test_1_download_jenkins_job(selfself):
        ...
        for link in soup.findAll('a'):
            ...
            if str(link.get('href')).endswith('.tar.gz'):
                ...
                os.system('wget -P ' + destination_dir + ' ' + tar_link+' > /dev/null')
        ...
        os.system('wget -P ' + destination_dir + ' ' + console_log_url+' > /dev/null')
        ...
        if len(ir_logs_urls)!=0:
            for url in ir_logs_urls:
                os.system('wget -P ' + destination_dir + ' ' + url+' > /dev/null')
        # Download tempest log (html #)
        if tempest_log_url!=None:
            os.system('wget -P ' + destination_dir + ' ' + tempest_log_url+' > /dev/null')
