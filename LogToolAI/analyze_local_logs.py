#!/usr/bin/python3
"""
Analyze logs in a user-provided local directory (no OpenShift).
Same pipeline as pod/must-gather: discover logs (recursive), baseline/since, extract blocks (threaded),
optional Ollama filter, report. Run from LogToolMain or directly.
"""
...
def discover_log_files(root_dir):
    """Recursively find all log files under root_dir (.log and .txt). Return (list of paths, root_dir)."""
    ...
    for dirpath, _dirnames, filenames in os.walk(root_dir):
        for name in filenames:
            ...
            if any(lower.endswith(ext) for ext in LOG_EXTENSIONS):
                log_paths.append(os.path.join(dirpath, name))
    return (sorted(log_paths), root_dir)


def component_from_path(root_dir, path):
    """Derive a group key from path: first directory under root, or 'root' if file is directly under root."""
    try:
        rel = os.path.relpath(path, root_dir)
        parts = rel.replace(os.sep, '/').split('/')
        if len(parts) >= 2:
            return parts[0].lower()
        return os.path.basename(os.path.dirpath(path)).lower() or 'root'
    except Exception:
        return 'other'
...
def main():
    ...
    if not path_in:
        ...
        sys.exit(1)
    root_dir = os.path.abspath(os.path.expanduser(path_in))
    if not os.path.isdir(root_dir):
        print(common.c(_YELLOW, 'Not a directory or not found: ') + root_dir)
        sys.exit(1)
    print(common.c(_GREEN, 'Using directory: ') + common.c(_CYAN, root_dir))

    print('')
    print(common.c(_CYAN, '[2/6] Discover log files'))
    print(common.c(_DIM, '-' * 60))
    log_paths, _ = discover_log_files(root_dir)
    if not log_paths:
        print(common.c(_YELLOW, 'No log files (.log, .txt) found under {}. Exiting.').format(root_dir))
        sys.exit(1)
    print(common.c(_GREEN, 'Found {} log files.').format(len(log_paths)))

    groups = group_logs_by_component(root_dir, log_paths)
    total = len(log_paths)
    num_options = len(groups) + 1
    print(common.c(_DIM, 'Choose which group to analyze (by subfolder):'))
    menu_items = []
    ...
    print(common.c(_DIM, '-' * 60))
    print(common.c(_DIM, 'Scanning last {} lines of selected log files for latest timestamp...').format(
        common.BASELINE_TAIL_LINES), flush=True)
    ...
    if use_ollama:
        ...
        if use_ollama:
            ...
            ollama_source = 'RHOSO (Red Hat OpenStack on OpenShift) / local log directory'
